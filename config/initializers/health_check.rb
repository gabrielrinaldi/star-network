...
HealthCheck.setup do |config|
  ...
  config.buckets = { 'armory-server-storage' => %i[R W D] }

  # You can customize which checks happen on a standard health check, eg to set an explicit list use:
  config.standard_checks = %w[database migrations cache redis]

  # You can set what tests are run with the 'full' or 'all' parameter
  full_checks = %w[cache database email migration redis sidekiq-redis site]
  config.full_checks = full_checks
  ...
  # Whitelist requesting IPs by a list of IP and/or CIDR ranges, either IPv4 or IPv6
  # Defaults to blank which allows any IP
  # config.origin_ip_whitelist = %w[123.123.123.123 10.11.12.0/24 2400:cb00::/32]

  # Use ActionDispatch::Request's remote_ip method when behind a proxy to pick up the real remote IP for
  # origin_ip_whitelist check otherwise uses Rack::Request's ip method (the default, and always used by Middleware),
  # which is more susceptable to spoofing
